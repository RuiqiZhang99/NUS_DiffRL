...
class PIT:
    ...
    def env_rollout(self, deterministic = False):
        ...
        # self.acc_returns = tu.compute_acc_returns(self.rew_buf, self.next_values, self.gamma_buf, self.done_mask, self.gamma)
    ...
    def compute_actor_loss(self):
        ...
        with torch.no_grad():
            ...
            gammas = actor_dataset["gamma"]
            rewards = actor_dataset["rewards"]
            next_values = actor_dataset["next_values"]

        l2a_grad_noise = torch.randn_like(l2a_grads) * 5e-7
        l2a_grads = l2a_grad_noise + l2a_grads
        ...
        acc_returns = tu.compute_acc_returns(rewards, next_values, gammas, done_mask, self.gamma) / (self.steps_num*self.num_envs)
        ratio = torch.exp(torch.sum(new_log_probs, dim=-1) - torch.sum(log_probs, dim=-1))
        # clipped_ratio = torch.clamp(ratio, 1-self.ppo_clip, 1+self.ppo_clip)
        with torch.no_grad():
            indiff_ratio = ratio.clone()
            # clipped_indiff_ratio = torch.clamp(indiff_ratio, 1-self.ppo_clip, 1+self.ppo_clip)

        surrogate_loss = (-torch.sum(diff_actions*l2a_grads, dim=-1)*indiff_ratio + acc_returns*ratio) 
        # surrogate_loss2 = (-torch.sum(diff_actions*l2a_grads, dim=-1)*clipped_indiff_ratio + acc_returns*clipped_ratio)
        
        self.epsilon = epsilon.mean()
        self.epsilon_his = self.epsilon_his + [epsilon.mean()]
    ...
    def train(self):
        ...
        for epoch in range(self.max_epochs):
            ...
            if len(self.episode_loss_his) > 0:
                ...
                self.writer.add_scalar('epsilon/step', self.epsilon, self.step_count)
                self.writer.add_scalar('epsilon/iter', self.epsilon, self.iter_count)
