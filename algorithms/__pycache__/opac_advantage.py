...
from utils.dataset import ActorDataset, CriticDataset
...
class OPAC:
    def __init__(self, cfg):
        ...
        self.horizon = cfg['params']['config'].get('horizon', 32)
        self.ppo_clip = cfg['params']['config'].get('ppo_clip', 0.2)
        self.num_minibatches = cfg['params']['config'].get('num_minibatches', 1)
        self.actor_interations = cfg['params']['config'].get('actor_interations', 1)
        self.policy_loss = cfg['params']['config'].get('policy_loss', 'advantage')
        ...
        self.next_obs_buf = torch.zeros((self.steps_num, self.num_envs, self.num_obs), dtype = torch.float32, device = self.device)
        ...
        self.deltas = torch.zeros((self.steps_num, self.num_envs), dtype = torch.float32, device = self.device)
        self.values = torch.zeros((self.steps_num, self.num_envs), dtype = torch.float32, device = self.device)
        ...
        self.action_buf = torch.zeros((self.steps_num, self.num_envs, self.num_actions), dtype = torch.float32, device = self.device)
        self.raw_action_buf = torch.zeros((self.steps_num, self.num_envs, self.num_actions), dtype = torch.float32, device = self.device)
        self.log_probs = torch.zeros((self.steps_num, self.num_envs, self.num_actions), dtype = torch.float32, device = self.device)
        self.gamma_buf = torch.zeros((self.steps_num, self.num_envs), dtype = torch.float32, device = self.device)
    ...
    def env_rollout(self, deterministic = False):

        rew_acc = torch.zeros((self.steps_num + 1, self.num_envs), dtype = torch.float32, device = self.device)
        deltas = torch.zeros((self.steps_num, self.num_envs), dtype = torch.float32, device = self.device)
        done_masks = torch.zeros((self.steps_num, self.num_envs), dtype = torch.float32, device = self.device)
        ...
        action_list = []
        # local_actions_buf.requires_grad_()
        ...
        for i in range(self.steps_num):
            ...
            self.time_report.start_timer("rollout forward")
            raw_actions = self.actor(obs, deterministic = deterministic)
            log_probs = self.actor.evaluate_actions_log_probs(obs, raw_actions)
            tanhed_actions = torch.tanh(raw_actions)
            tanhed_actions.requires_grad_()
            self.time_report.end_timer("rollout forward")

            action_list = action_list + [tanhed_actions]
            with torch.no_grad():
                self.action_buf[i] = tanhed_actions.clone()
                self.raw_action_buf[i] = raw_actions.clone()

            self.time_report.start_timer("rollout step")
            obs, rew, done, extra_info = self.env.step(tanhed_actions)
            self.time_report.end_timer("rollout step")
            
            with torch.no_grad():
                raw_rew = rew.clone()
                self.action_buf[i] = tanhed_actions.clone()
                self.raw_action_buf[i] = raw_actions.clone()
                self.log_probs[i] = log_probs.clone()            
            ...
            rew_acc[i + 1, :] = gamma * rew
            delta = rew + self.gamma * next_values[i + 1, :] + next_values[i]
            deltas[i] = delta
            with torch.no_grad():
                self.deltas[i] = delta.clone()
            actor_loss = actor_loss + (-delta * gamma).sum()
            gamma = gamma * self.gamma * self.lam
            ...
            if i < self.steps_num - 1:
                done_masks[i] = done.clone().to(torch.float32)
            else:
                done_masks[i, :] = 1.
        ...
        advantages = tu.compute_advatages(deltas, self.gamma, self.lam, done_masks)
                            
        if self.ret_rms is not None:
            actor_loss = actor_loss * torch.sqrt(ret_var + 1e-6)

        self.time_report.start_timer("rollout backward")
        l2a_grads = torch.cat(torch.autograd.grad(actor_loss, tuple(action_list), retain_graph=True), dim=0)
        
        with torch.no_grad():
            l2a_grads = l2a_grads.reshape(self.steps_num, self.num_envs, self.num_actions)
            l2a_grads = l2a_grads * torch.repeat_interleave(self.gamma_buf.unsqueeze(-1), self.num_actions, dim=-1)
            self.values = next_values[:-1].clone()
            self.advantages = advantages.clone()
        self.l2a_grads = l2a_grads
        self.time_report.end_timer("rollout backward")
    ...
    def compute_actor_loss(self):
        actor_dataset = self.actor_dataset
        with torch.no_grad():
            indiff_actions = actor_dataset["actions"]
            raw_actions = actor_dataset["raw_actions"]
            obs = actor_dataset["obs"]
            l2a_grads = actor_dataset["l2a_grads"]
            log_probs = actor_dataset["log_probs"]
            done_mask = actor_dataset["done_mask"]
            gamma = actor_dataset["gamma"]
            deltas = actor_dataset['deltas']
            rewards = actor_dataset["rewards"]
            values = actor_dataset["values"]
            next_values = actor_dataset["next_values"]
            advantages = actor_dataset["advantages"]

        _, mu, std = self.actor.forward_with_dist(obs.view(-1, obs.shape[-1]))
        mu = mu.reshape(self.horizon, -1, self.num_actions)
        std = std.repeat_interleave(self.horizon * self.num_minibatches * self.num_envs, dim=0).reshape(self.horizon, self.num_minibatches * self.num_envs, self.num_actions)
        with torch.no_grad():
            epsilon = (raw_actions - mu) / std
        diff_raw_actions = mu + epsilon * std
        new_log_probs = self.actor.evaluate_actions_log_probs(obs, diff_raw_actions)
        diff_actions = torch.tanh(diff_raw_actions)
        
        done_idx = done_mask.nonzero()
        # with torch.no_grad():
            # importance_weight = torch.exp(new_log_probs - log_probs)
        ppo_ratio = torch.exp(torch.sum(new_log_probs, dim=-1) - torch.sum(log_probs, dim=-1))
        with torch.no_grad():
            indiff_ppo_ratio = ppo_ratio.clone()

        surrogate_loss1 = (torch.sum(diff_actions*l2a_grads, dim=-1)*indiff_ppo_ratio - advantages*ppo_ratio) / (self.num_envs*self.steps_num)
        # surrogate_loss2 = -(torch.sum(diff_actions*l2a_grads, dim=-1)*torch.clamp(indiff_ppo_ratio, 1-self.ppo_clip, 1+self.ppo_clip)+advantages*torch.clamp(ppo_ratio, 1-self.ppo_clip, 1+self.ppo_clip)) / (self.num_envs*self.steps_num)
        

        if self.steps_num != self.horizon:
            grad_scale = (torch.ones_like(gamma, dtype = torch.float32, device = self.device) / (gamma[0, :].unsqueeze(0))).nan_to_num_(0.0, 0.0, 0.0)
            for i in range(done_idx.shape[0]):
                grad_scale[done_idx[i, 0]+1:, done_idx[i, 1]] = 1.0
            
            surrogate_loss1 = surrogate_loss1 * grad_scale
            # surrogate_loss2 = surrogate_loss2 * grad_scale
        
        else:
            pass
        diff_sim_loss = surrogate_loss1.mean()
        # diff_sim_loss = -torch.min(surrogate_loss1, surrogate_loss2).mean()

        '''
        ppo_ratio = torch.exp(torch.sum(new_log_probs, dim=-1) - torch.sum(log_probs, dim=-1))
        deltas = rewards + self.gamma * next_values * (1-done_mask) - values
        advantages = torch.flip(torch.cumsum(torch.flip(deltas * gamma, [0]), dim=0), [0])
        surrogate_loss1_ppo = advantages * ppo_ratio
        surrogate_loss2_ppo = torch.clamp(advantages * ppo_ratio, 1-self.ppo_clip, 1+self.ppo_clip)
        ppo_loss = -torch.min(surrogate_loss1_ppo, surrogate_loss2_ppo).mean()'''

        actor_loss = diff_sim_loss
        return actor_loss
    ...
    def train(self):
        ...
        self.time_report.add_timer("trajectory rollout")
        self.time_report.add_timer("rollout forward")
        self.time_report.add_timer("rollout backward")
        self.time_report.add_timer("rollout step")
        ...
        for epoch in range(self.max_epochs):
            ...
            self.time_report.start_timer("trajectory rollout")
            self.env_rollout()
            buffer_info = {
                'obs': self.obs_buf, 
                'next_obs': self.next_obs_buf,
                'raw_actions': self.raw_action_buf,
            ...
            self.time_report.end_timer("trajectory rollout")
            # train actor
            for i in range(self.actor_interations):
                with torch.no_grad():
                    # actor_dataset = ActorDataset(self.horizon, buffer_info)
                    self.actor_dataset = buffer_info
