...
class OPAC:
    def __init__(self, cfg):
        ...
        self.r2a_grads = torch.zeros((self.steps_num, self.steps_num, self.num_envs, self.num_actions), dtype = torch.float32, device = self.device)
        self.nv2a_grads = torch.zeros((self.steps_num, self.steps_num, self.num_envs, self.num_actions), dtype = torch.float32, device = self.device)
    ...
    def env_rollout(self, deterministic = False):
        ...
        action_list, reward_list, next_value_list = [], [], []
        ...
        init_value = self.target_critic(obs).squeeze(-1)
        ...
        for i in range(self.steps_num):
            ...
            next_value_t = self.target_critic(obs).squeeze(-1)
            next_values[i + 1] = next_value_t
            ...
            for id in done_env_ids:
                if torch.isnan(extra_info['obs_before_reset'][id]).sum() > 0 \
                    ...
                    real_next_value = self.target_critic(real_obs).squeeze(-1)
                    next_value_t[id] = real_next_value
                    next_values[i + 1, id] = real_next_value
            ...
            value_sum_t = torch.sum(next_value_t)
            next_value_list = next_value_list + [value_sum_t]
            reward_sum_t = torch.sum(rew * (1 - done))
            reward_list = reward_list + [reward_sum_t]
            self.time_report.start_timer("rollout backward")
            r2a_grads = torch.cat(torch.autograd.grad(reward_sum_t, tuple(action_list), retain_graph=True), dim=0)
            nv2a_grads = torch.cat(torch.autograd.grad(value_sum_t, tuple(action_list), retain_graph=True), dim=0)
            self.time_report.end_timer("rollout backward")

            with torch.no_grad():
                r2a_grads = r2a_grads.reshape(i+1, self.num_envs, self.num_actions)
                nv2a_grads = nv2a_grads.reshape(i+1, self.num_envs, self.num_actions)
                self.r2a_grads[i, :i+1] = r2a_grads.clone()
                self.nv2a_grads[i, :i+1] = nv2a_grads.clone()
    ...
    def compute_actor_loss(self):
        ...
        with torch.no_grad():
            ...
            r2a_grads = actor_dataset["r2a_grads"]
            nv2a_grads = actor_dataset["nv2a_grads"]

            log_probs = actor_dataset["log_probs"]
            target_values = actor_dataset["target_values"]
        ...
        mu = mu.reshape(self.horizon, self.num_envs, self.num_actions)
        std = std.repeat_interleave(self.horizon * self.num_envs, dim=0).reshape(self.horizon, self.num_envs, self.num_actions)
        with torch.no_grad():
        ...
        # discounting_tensor = torch.logspace(0, self.horizon-1, self.horizon, base=self.gamma*self.lam, device=self.device).reshape(-1, 1, 1, 1)
        surrogate_rewards = torch.sum(torch.sum(r2a_grads  , dim=0) * diff_actions, dim=-1)
        surrogate_next_values = torch.sum(torch.sum(nv2a_grads, dim=0) * diff_actions, dim=-1)
        delta = surrogate_rewards + self.gamma * (1 - done_mask) * surrogate_next_values - values

        adv_list = []
        adv_t = torch.zeros((self.num_envs), dtype = torch.float32, device = self.device)
        for t in reversed(range(self.horizon)):
            adv_t = delta[t] + self.gamma * self.lam * (1.0 - done_mask)[t] * adv_t
            adv_list = adv_list + [adv_t]
        adv = torch.stack(list(reversed(adv_list)))

        # actor_loss = torch.sum(diff_actions * l2a_grads)
        importance_weight = torch.exp(torch.sum(new_log_probs, dim=-1) - torch.sum(log_probs, dim=-1))
        surrogate_loss1 = -adv * importance_weight
        surrogate_loss2 = -adv * torch.clamp(importance_weight, 1-self.ppo_clip, 1+self.ppo_clip)
        ...
        for epoch in range(self.max_epochs):
            ...
            for i in range(self.actor_interations):
                ...
                buffer_info = {
                    ...
                    'r2a_grads': self.r2a_grads,
                    'nv2a_grads': self.nv2a_grads,
                ...
                with torch.no_grad():
                    ...
                    self.actor_dataset = actor_dataset.sample_horizon(seed)
