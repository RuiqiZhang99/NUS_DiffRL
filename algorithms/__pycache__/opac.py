...
class OPAC:
    def __init__(self, cfg):
        ...
        self.num_minibatches = cfg['params']['config'].get('num_minibatches', 4)
    ...
    def env_rollout(self, deterministic = False):
        ...
        adv = torch.zeros((self.steps_num, self.num_envs), dtype = torch.float32, device = self.device)
        ...
        for i in range(self.steps_num):
            ...
            if self.policy_loss == 'discounted_reward':
            ...
            elif self.policy_loss == 'advantage':
                rew_acc[i + 1, :] = gamma * rew
                actor_loss = actor_loss + (- rew_acc[i + 1, :] - self.gamma * gamma * next_values[i + 1, :] + gamma * next_values[i]).sum()
    ...
    def compute_actor_loss(self):
        ...
        if self.steps_num != self.horizon:
            ...
            # grad_mask = (torch.cumsum(done_mask, dim=0) > 0).to(torch.float32)
            # grad_scale = gamma * (1.0 - grad_mask) + torch.ones_like(gamma) * grad_mask
            # actor_loss = torch.sum(diff_actions * l2a_grads)
            surrogate_loss1 = torch.sum(-diff_actions * l2a_grads * importance_weight, dim=-1) * grad_scale
            surrogate_loss2 = torch.sum(-diff_actions * l2a_grads * torch.clamp(importance_weight, 1-self.ppo_clip, 1+self.ppo_clip), dim=-1) * grad_scale
        
        else: 
            surrogate_loss1 = torch.sum(-diff_actions * l2a_grads * importance_weight, dim=-1)
            surrogate_loss2 = torch.sum(-diff_actions * l2a_grads * torch.clamp(importance_weight, 1-self.ppo_clip, 1+self.ppo_clip), dim=-1)
        
        diff_sim_loss = -torch.min(surrogate_loss1, surrogate_loss2).sum()
    ...
    def train(self):
        ...
        for epoch in range(self.max_epochs):
            ...
            for i in range(self.actor_interations):
                # seed = random.randint(0, self.steps_num-self.horizon-1)
                with torch.no_grad():
                    actor_dataset = ActorDataset(self.horizon, buffer_info)
                    self.actor_dataset = actor_dataset.sample_horizon(self.num_minibatches)
